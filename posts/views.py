...
def useful(request):
    ...
    return render(request, 'posts/Useful_Information.html', context)
...
def detail_faq(request, post_id):
    ...
    return render(request, 'posts/detail.html', {'post': faq})


def detail_useful(request, post_id):
    useful = get_object_or_404(Useful, pk=post_id)
    return render(request, 'posts/detail.html', {'post': useful})